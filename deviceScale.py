...
class _WMWebService:
    ...
    def __init__(self, cfg: dict):
        ...
        # Service de notifications (GetNotifications)
        ...
        try:
            if self.door_ids:
                req = {
                    ...
                    "DraftShieldIds": {"DraftShieldIdentifier": [self.door_ids[0]]},
                }
                _ = self._svc_draft.GetPosition(**req)
            return True
        except Exception:
            return False
    ...
    def open_door(self) -> str:
        ...
        def _set_all():
            for d in self.door_ids:
                self._set_position_one(d, self.open_width)
        ...
        if self._verify_door(self.open_width):
            return "OK"

        # 2) échec → poke écran puis retente
        self._poke_display()
        _set_all()
        if self._verify_door(self.open_width):
            return "OK (after wake)"

        # 3) toujours pas → remonter une erreur claire
        raise RuntimeError("Open door: mouvement non observé (écran en veille ou verrouillage UI ?)")

    def close_door(self) -> str:
        self._ensure_session()
        def _set_all():
            for d in self.door_ids:
                self._set_position_one(d, self.close_width)
        ...
        if self._verify_door(self.close_width):
            return "OK"
        ...
        raise RuntimeError("Close door: mouvement non observé (écran en veille ou verrouillage UI ?)")
    ...
    def _poke_display(self):
        """Envoie une requête inoffensive qui 'réveille' l’UI (screen saver) côté balance."""
        self._ensure_session()
        try:
            # Un GetWeight en Immediate suffit typiquement à 'réveiller' l’UI.
            _ = self._svc_weigh.GetWeight(
                SessionId=self.session_id,
                WeighingCaptureMode="Immediate",
                TimeoutInSeconds=1,
            )
        except Exception:
            # Même si ça échoue, on ne propage pas forcément : l’objectif est juste de 'poke'
            pass

    def _draft_positions(self) -> dict[str, int]:
        """Retourne {draft_id: opening_width}."""
        req = {
            "SessionId": self.session_id,
            "DraftShieldIds": {"DraftShieldIdentifier": self.door_ids},
        }
        resp = self._svc_draft.GetPosition(**req)
        out: dict[str, int] = {}
        # Zeep peut renvoyer liste/objet
        dsps = getattr(resp, "DraftShieldsPositions", None)
        if dsps is None and isinstance(resp, dict):
            dsps = resp.get("DraftShieldsPositions")
        if not dsps:
            return out
        items = getattr(dsps, "DraftShieldPosition", None) or dsps.get("DraftShieldPosition") or []
        if not isinstance(items, (list, tuple)):
            items = [items]
        for it in items:
            did = getattr(it, "DraftShieldId", None) if not isinstance(it, dict) else it.get("DraftShieldId")
            w   = getattr(it, "OpeningWidth", None)  if not isinstance(it, dict) else it.get("OpeningWidth")
            if did is not None and w is not None:
                try:
                    out[str(did)] = int(w)
                except Exception:
                    pass
...
class WM:
    ...
    def get_dosing_head_name(self) -> str:
        return self._impl.read_dosing_head_name()

    def set_dosing_head_name(self, name: str) -> str:
        return self._impl.write_dosing_head_name(name)